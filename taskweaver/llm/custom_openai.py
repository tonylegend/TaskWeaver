...
class OpenAIServiceConfig(LLMServiceConfig):
    def _configure(self) -> None:
        ...
        self.session_token = self._get_str("session_token", "")
        google_api_key = self._get_str("google_api_key", "")
...
class OpenAIService(CompletionService, EmbeddingService):
    ...
    def chat_completion(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            kwargs['bot_id'] = conversation.bot_id
            ...
            if stream and messages:
                ...
                for stream_res in res:
                    if isinstance(stream_res, ChatCompletion) and stream_res.choices:
                        message = stream_res.choices[0].message
                        role = message.role
                        content = message.content
                        if hasattr(stream_res, 'bot_id') and stream_res.bot_id != conversation.bot_id:
                            conversation.bot_id = stream_res.bot_id
                    elif isinstance(stream_res, ChatCompletionChunk) and stream_res.choices:
                ...
                if hasattr(res, 'bot_id') and res.bot_id != conversation.bot_id:
                    conversation.bot_id = res.bot_id
    ...
    #         # create random embeddings for each string in inputs
    #
    #     embedding_results = [random_floats() for _ in strings]
